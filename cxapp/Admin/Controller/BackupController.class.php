...
class BackupController extends \Admin\Controller\AdminController {
	...
	public function _initialize() {
		$this->backup_path = APP_DATA_PATH . 'backup/';
		parent::_initialize();
	}
...
//		$allow_max_size = $this->_return_bytes(@ini_get('upload_max_filesize'));
	...
	public function import() {
		...
		if ($this->_import_vol($backup['file'])) {
			if ($vol < count($backups)) {
				$vol++;
				$link = __CONTROLLER__ . '/import/vol/' . $vol . '/backup/' . urlencode($this->backup_name);
				$this->success(sprintf('导入成功！', $vol - 1), $link);
			} else {
				$this->success('导入成功！', __CONTROLLER__ . '/restore');
			}
		}
	}
	...
	public function del_backup() {
		...
		$dir = new \Common\Lib\Util\Dir();
		$dir->delDir($this->backup_path . $_GET['backup'] . '/');
		$this->success('操作成功！');
	}
	...
	private function _deal_result($tables, $vol, $sizelimit) {
		...
		if (empty($tables)) {
			...
			$this->success('备份成功！', U($this->getLowerTplName('restore')));
		} else {
			//开始下一卷
			$this->_set_vol($vol); //设置分卷记录
			$link = __CONTROLLER__ . '/index/dosubmit/1/type/url/backup_name/' . $this->backup_name . '/sizelimit/' . $sizelimit;
	...
	private function _get_table_df($table) {
		...
		$tmp_sql = M()->query("SHOW CREATE TABLE `$table` ");
	...
	private function _get_table_data($table, $pos, $sizelimit) {
		$post_pos = $pos;
		$total = M()->query("SELECT COUNT(*) FROM $table"); //数据总数
		$total = $total[0]['COUNT(*)'];
		...
		for ($i = 0; $i < $cycle_time; $i++) {
			$data = M()->query("SELECT * FROM $table LIMIT " . ($this->
	...
	private function _put_tbl_queue($tables) {
		return file_put_contents($this->backup_path . $this->backup_name . '/tbl_queue.log', "<?php return " . var_export($tables, true) . ";\n?>");
	}
	...
	private function _get_tbl_queue() {
		$tbl_queue_file = $this->backup_path . $this->backup_name .
				'/tbl_queue.log';
	...
	private function _sava_sql($vol) {
		return file_put_contents($this->backup_path . $this->backup_name . '/' . $this->backup_name . '_' . $vol . '.sql', $this->dump_sql);
	}
