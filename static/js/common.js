...
(function() {
	...
	$(document).on('keydown', function(event) {
		...
		if (e.keyCode === 116) {
			...
			var id = $doc.find('#B_history .current').attr('data-id');
			var iframe = $doc.find('#iframe_' + id);
			try {
	...
	if (color_pick.length) {
		Wind.use('colorPicker', function() {
			color_pick.each(function() {
				$(this).colorPicker({
					default_color: 'url("' + GV.DIMAUB + 'static/images/transparent.png")', //写死
					callback: function(color) {
						var em = $(this).find('em');
						var input = $(this).next('.J_hidden_color');
	...
	if ($('.J_font_config').length) {
		Wind.use('colorPicker', function() {
			...
			elem.each(function() {
				...
				$(this).colorPicker({
					default_color: 'url("' + GV.DIMAUB + 'static/images/transparent.png")',
					callback: function(color) {
